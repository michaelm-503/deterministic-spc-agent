...
def _best_run_match(prompt: str, runs: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if best_score < 0.40:
        ...
        raise ValueError(
            ...
            "Phase 4A currently supports a constrained prompt set only. "
...
def generate_plan_from_prompt(
    ...
    *,
    ...
    """
    Phase 4A planner abstraction.

    Current implementation is intentionally constrained:
    - loads a known plan library from planner/demo_gallery.json
    - matches the incoming natural-language prompt to a supported request_text
    - returns the corresponding deterministic run plan
    """
    ...
    matched_run = _best_run_match(prompt, runs)

    return PlannerResult(
        prompt=prompt,
        plan=matched_run,
        matched_request_text=matched_run.get("request_text", "<missing request_text>"),
        source_path=str(planner_path),
    )
